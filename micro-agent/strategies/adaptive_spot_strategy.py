...
class AdaptiveSpotStrategy:
    def __init__(
        ...
        # Direction multiplier for long/short bias
        self.direction = direction
        self.target = target
        # Initialize EWMA regression for online learning
    ...
    def update(
        ...
        if np.isnan(x).any() or np.isnan(forward_log_return):
            return

        # Train regression on observed return
        self.ewma_reg.update(x, forward_log_return)
